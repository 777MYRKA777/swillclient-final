...
public class SwillClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        System.out.println("[SWILL] Client initialized for Minecraft 1.21.5");
        
        // Регистрация клавиши меню (RSHIFT)
        menuKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
            "key.swill.menu",
            InputUtil.Type.KEYSYM,
            GLFW.GLFW_KEY_RIGHT_SHIFT,
            "category.swill.client"
        ));
        
        // Обработчик тиков (аимбот + тригербот)
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (client.player == null) return;
            
            // Открытие/закрытие меню по RSHIFT
            if (menuKey.wasPressed()) {
                menuOpen = !menuOpen;
                if (client.player != null) {
                    client.player.sendMessage(Text.literal("§6[SWILL] §7Menu " + (menuOpen ? "§aOpened" : "§cClosed")), true);
                }
            }
            ...
            if (triggerbotEnabled && client.crosshairTarget instanceof EntityHitResult) {
                ...
                if ((target instanceof HostileEntity || target instanceof PassiveEntity) && target.isAlive()) {
                    if (client.player.distanceTo(target) <= 4.2) {
    ...
    private void aimAtEntity(MinecraftClient client, Entity target) {
        ...
        // Плавное наведение (коэффициент 0.5 = мгновенно, выше = плавнее)
    ...
    private void drawEntityESP(net.fabricmc.fabric.api.client.rendering.v1.WorldRenderContext context, Entity entity) {
        ...
        // Цвет: красный для враждебных, зелёный для пассивных
